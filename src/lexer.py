...
# 定义 Token 类型及其正则表达式模式
TOKEN_SPECIFICATION = [
    ('KEYWORD', r'\b(if|else|for|while|return|int|float|double|char|bool|void)\b'),  # 关键字
    ('BUILTIN_FUNC', r'\b(printf|scanf)\b'),   # 内置函数
    ('IDENTIFIER', r'[A-Za-z_]\w*'),           # 标识符
    ('HEX_LITERAL', r'0[xX][0-9a-fA-F]+'),     # 十六进制
    ('OCT_LITERAL', r'0[oO]?[0-7]+'),          # 八进制
    ('BIN_LITERAL', r'0[bB][01]+'),            # 二进制
    ('FLOAT_LITERAL', r'\d+\.\d+([eE][+-]?\d+)?'),  # 浮点数
    ('INT_LITERAL', r'\d+'),                   # 十进制整数
    ('STRING_LITERAL', r'"(\\.|[^"\\])*"'),    # 字符串常量
    ('CHAR_LITERAL', r"'(\\.|[^'\\])'"),       # 字符常量
    ('OPERATOR', r'(\+\+|--|\+=|-=|\*=|/=|==|!=|<=|>=|&&|\|\||[+\-*/=<>&|!])'),  # 操作符
    ...
    def tokenize(self):
        ...
        for match in re.finditer(TOKEN_REGEX, self.code):
            ...
            if kind == 'KEYWORD':
                ...
                value = int(value)
            elif kind in ('STRING_LITERAL', 'CHAR_LITERAL'):
